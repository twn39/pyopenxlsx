...
void init_rich_text(py::module_& m) {
    ...
    py::class_<XLRichTextRun>(m, "XLRichTextRun")
        ...
        .def_prop_rw(
            ...
            [](XLRichTextRun& self, const py::object& name) {
                if (name.is_none())
                    self.setFontName(
                        "");  // OpenXLSX doesn't have a clear font name, but we can set empty
                else
                    self.setFontName(py::cast<std::string>(name));
            })
        .def_prop_rw(
            "font_size",
            [](const XLRichTextRun& self) -> py::object {
            ...
            [](XLRichTextRun& self, const py::object& size) {
                if (size.is_none()) {
                    ...
                    self.setFontSize(py::cast<size_t>(size));
            })
        .def_prop_rw(
            "font_color",
            [](const XLRichTextRun& self) -> py::object {
            ...
            [](XLRichTextRun& self, const py::object& color) {
                if (color.is_none()) {
                    ...
                    self.setFontColor(py::cast<XLColor>(color));
            })
        .def_prop_rw(
            "bold",
            [](const XLRichTextRun& self) -> py::object {
        ...
        .def_prop_rw(
            ...
            [](XLRichTextRun& self, const py::object& u) {
                if (u.is_none()) {
                    ...
                    self.setUnderline(py::cast<bool>(u));
            })
        .def_prop_rw(
            "strikethrough",
            [](const XLRichTextRun& self) -> py::object {
            ...
            [](XLRichTextRun& self, const py::object& s) {
                if (s.is_none()) {
                    ...
                    self.setStrikethrough(py::cast<bool>(s));
            });
    ...
    py::class_<XLRichText>(m, "XLRichText")
        ...
        .def_prop_ro(
            "runs",
            [](XLRichText& self) {
                return py::make_iterator(py::type<XLRichText>(), "iterator", self.runs().begin(),
                                         self.runs().end());
            },
            py::keep_alive<0, 1>())
