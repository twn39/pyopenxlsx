...
void init_formula_engine(py::module_& m) {
    py::class_<XLFormulaEngine>(m, "XLFormulaEngine")
        .def(py::init<>())
        .def("evaluate", [](const XLFormulaEngine& self, std::string_view formula, const XLWorksheet* wks) -> py::object {
            XLCellValue result;
            if (wks) {
        ...
        "Evaluate a formula string. Optionally provide an XLWorksheet to resolve cell references.");
}
