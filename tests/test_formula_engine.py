...
def test_formula_engine_excelize_cases():
    ...
    for formula, expected in test_cases.items():
        ...
        if (
            ...
            assert result == pytest.approx(expected, rel=1e-4, abs=1e-9), (
                f"Failed: {formula}"
            )
        else:
            assert result == expected, (
                f"Failed: {formula} -> expected {expected}, got {result}"
            )
    # Test error cases
    error_cases = {
    ...
    for formula in error_cases:
        try:
            ...
            assert (
                ...
                and "ERROR" in result.upper()
                or type(result).__name__ == error_cases[formula]
            )
        except Exception:
